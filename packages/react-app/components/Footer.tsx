      ...
      <footer className="bg-gray15 mt-auto border-black border-t">
        <div className="mx-auto max-w-7xl py-6 px-4 sm:px-6 md:flex md:items-center md:justify-between lg:px-8">
          <div className="flex justify-center space-x-6 md:order-2">
            {navigation.map((item) => (
              <a key={item.name} href={item.href} className="text-yellow-300 hover:text-forest" target="_blank" rel="noopener noreferrer">
          ...
          <div className="mt-8 md:order-1 md:mt-0">
            <p className="text-center text-base text-neutral-400">&copy; {new Date().getFullYear()} Developed with 💛 by Kayzee.</p>
          </div>
        </div>
      </footer>
    )
